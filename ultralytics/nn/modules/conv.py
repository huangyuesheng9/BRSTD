...
class XYWA_f(nn.Module):
    def __init__(self, c1, ratio=16):
        ...
        self.dilc11 = Xc(c1, c1)
        self.dilc12 = Xc(c1, 1)
        self.dilc21 = Yc(c1, c1)
        self.dilc22 = Yc(c1, 1)
        self.dilc31 = W(c1, c1)
        self.dilc32 = W(c1, 1)
        self.conv1 = nn.Conv2d(c1, c1, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        s1 = self.dilc12(self.dilc11(x1))
        s2 = self.dilc22(self.dilc21(x1))
        s3 = self.dilc32(self.dilc31(x1))
        s = s1+s2+s3
        c = self.fc2(self.relu(self.fc1(self.avg_pool(x1))))
        sc = self.sigmoid(s * c)
...
class XYWConv_f(nn.Module):
    def __init__(self, c1, r=1):
        super(XYWConv_f, self).__init__()
        self.dilc11 = Xc(c1, c1)
        self.dilc12 = Xc(c1, c1)
        self.dilc21 = Yc(c1, c1)
        self.dilc22 = Yc(c1, c1)
        self.dilc31 = W(c1, c1)
        self.dilc32 = W(c1, c1)
    def forward(self,x):
        s1 = self.dilc12(self.dilc11(x))
        s2 = self.dilc22(self.dilc21(x))
        s3 = self.dilc32(self.dilc31(x))
        out = s1 + s2 + s3 + x
        return out

class XYWConv_xxx(nn.Module):
    def __init__(self, c1, r=1):
        super(XYWConv_xxx, self).__init__()
        self.dilc11 = nn.Conv2d(c1, c1, kernel_size=3, padding=1, groups=c1)
        self.dilc12 = nn.Conv2d(c1, c1, kernel_size=3, padding=1, groups=c1)
        self.dilc21 = nn.Conv2d(c1, c1, kernel_size=3, padding=1, groups=c1)
        self.dilc22 = nn.Conv2d(c1, c1, kernel_size=3, padding=1, groups=c1)
        self.dilc31 = W(c1, c1)
        self.dilc32 = W(c1, c1)
    def forward(self,x):
        s1 = self.dilc12(self.dilc11(x))
